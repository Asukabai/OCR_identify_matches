...
//public class Table {
//    @Resource
//    private static ConfigTypesMapper configTypesMapper;
//    @Resource
//    private static ConfigItemsMapper configItemsMapper;
//
...
//        nameAliases = configItemsMap.getOrDefault("名称", Collections.emptyList()).stream().map(ConfigItems::getAlias).collect(Collectors.toList());
//        specAliases = configItemsMap.getOrDefault("型号", Collections.emptyList()).stream().map(ConfigItems::getAlias).collect(Collectors.toList());
//    }
//
//    /**
//    * 重要提示代码中所需工具类
