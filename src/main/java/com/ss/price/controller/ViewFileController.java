...
//        // 打印查询字符串
//        log.info("打印查询字符串：Search query: {}", operation.getReqData().getQuery());
//        queryWrapper.apply("MATCH(content) AGAINST({0})",  operation.getReqData().getQuery())
//                .or()
//                .like("file_name", operation.getReqData().getQuery())
//                .orderByDesc("id");
//        // 打印生成的 SQL 语句
...
//                log.info("第一次查询没有结果，使用 likeQueryWrapper 进行第二次分页查询");
//            QueryWrapper<FileInfo> likeQueryWrapper = new QueryWrapper<>();
//            likeQueryWrapper.like("content", operation.getReqData().getQuery())
//                    .or()
//                    .like("file_name", operation.getReqData().getQuery())
//                    .orderByDesc("id");
//            // 打印第二次查询的 SQL 语句
...
//        // 打印查询字符串
//        log.info("打印查询字符串：Search query: {}", operation.getReqData().getQuery());
//        queryWrapper.like("content", operation.getReqData().getQuery())
//                .or()
//                .like("file_name", operation.getReqData().getQuery())
//                .orderByDesc("id");
//        // 打印生成的 SQL 语句
...
//            QueryWrapper<FileInfo> likeQueryWrapper = new QueryWrapper<>();
//            likeQueryWrapper.like("file_name", operation.getReqData().getQuery())
//                    .orderByDesc("id");
//            // 打印第二次查询的 SQL 语句
//            log.info("打印生成的 SQL 语句：Generated SQL: {}", likeQueryWrapper.getSqlSegment());
//            try {
...
//    }
    ...
    // 优化点：
    // 如果多个用户同时访问此接口，可能会导致数据库连接池耗尽或文件系统 I/O 瓶颈 (并发访问处理)线程池: 使用线程池管理并发任务，避免系统资源被过度占用。
    // 负载均衡: 在高并发场景下，考虑使用负载均衡技术，分散请求压力。
    // 分页查询: 确保分页查询的参数合理，避免一次性查询大量数据。
    // 缓存机制: 考虑引入缓存机制（如 Redis），将频繁查询的结果缓存起来，减少数据库查询次数。
    // 异步处理: 将图片转换为 Base64 字符串的操作异步化，避免阻塞主线程。
...
//        Page<FileInfo> page = new Page<>(operation.getReqData().getPageNum(), operation.getReqData().getPageSize());
//        QueryWrapper<FileInfo> queryWrapper = new QueryWrapper<>();
//        // 这是一个全文搜索查询，用于在 content 字段中搜索与 {0} 匹配的内容。{0} 是一个占位符，会被 operation.getReqData().getQuery() 替换。
//        queryWrapper.apply("MATCH(content) AGAINST({0})", operation.getReqData().getQuery())
//                .or()
//                .like("file_name", operation.getReqData().getQuery())
//                .orderByDesc("id"); // 查询所有的 FileInfo 记录，并进行倒序排序
//        Page<FileInfo> fileInfoPage = fileInfoMapper.selectPage(page, queryWrapper);
//        // 将查询到的 FileInfo 记录封装到 FileInfoResponse 对象中
//        List<FileInfoResponse> fileInfoResponseList = new ArrayList<>();
//        for (FileInfo fileInfo : fileInfoPage.getRecords()) {
...
//    }
    ...
    // 分页返回所有的查询记录   /ss/service/getAllFileInfoWithImages
